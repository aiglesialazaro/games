...
class Design_4_4_JumpandRunMovement extends ActorScript
{
	...
	override public function init()
	{
		...
		addWhenUpdatedListener(null, function(elapsedTime:Float, list:Array<Dynamic>):Void
		{
			if(wrapper.enabled)
			{
				if(!(Engine.engine.getGameAttribute("Is swinging sword")))
				{
					if((Engine.engine.getGameAttribute("sceneTransitioning") == false))
					{
						Engine.engine.setGameAttribute("Face Right", !(_FacingLeft));
						if(_UseControls)
						{
